...
ssh "$SSH_ALIAS" bash <<EOF
    ...
    overall=0
    wait_healthy "API Gateway" "http://localhost:$PROD_API_GATEWAY_PORT/health" || overall=1
    wait_healthy "Data Server" "http://localhost:$PROD_DATA_SERVER_PORT/health"  || overall=1
    wait_healthy "Frontend"    "http://localhost:$PROD_FRONTEND_PORT"            || overall=1
    exit \$overall
EOF
